...
%This is described below:(1)  The paper discusses a piano(2)The prototype is meant for playing, learning, or teachingthe piano(3)  The prototype is an augmented pian

\section{Technology trends in augmentation}

We analyzed sixty-one (61) augmented piano prototypes which were published within the recent two decades. Part of the analysis is to uncover if these prototypes are related to each other or may have inspired the design of other prototypes and their specific features. We also draw light to the different venues where these papers have been published and discussed. These can be found in Table \ref{tab:venues}. We wanted to find out whether these augmented pianos have been shared in high-quality venues on HCI and Music Interfaces such as the ACM CHI Conference on Human Factors in Computing Systems (CHI); the ACM SIGCHI Annual Symposium on Computer-Human Interaction in Play (CHI PLAY); the ACM Symposium on User Interface Software and Technology (UIST) \cite{sharma2017framework}; the International Symposium on Mixed and Augmented Reality (ISMAR) \cite{dey2016systematic, zhou2008trends}; the International Conference on New Interfaces for Musical Expression (NIME) \cite{jensenius2017nime}. 
...
Apart from these augmentation trends, we introduce learner-based themes that we believe have inspired these contributions. These themes were extracted after a qualitative analysis, drawing insights from how the piano learning process takes place. While there are many existing music education methodologies (Kodaly, Suzuki, etc) and relevant learning frameworks (SLT, ExL, etc) that were considered by some of these papers, we believe that these learner-based themes capture a more fitting perspective of these augmented piano prototypes especially in the context of piano learning. These learner-based themes are as follows: 

\subsection{Ensuring correct finger positioning for novice learners}

Using the traditional piano involves several motor skills such as using both left and right hands to press keys, maintaining proper posture and coordinating leg and foot movement when pressing on the foot pedals of the piano. These motor skills are used when a user plays a piece in the piano. In order to produce proper sound, timing also plays an important role. All in all, these skills, when well-trained allows the user to play produce a melody that is not off-tune and is pleasant to the audience's ears. However, most novices struggle in performing this task not to consider their physical attributes (slim vs fat fingers, long vs short legs) that might also hinder them from doing this task properly. This is why most traditional music teaching frameworks introduced various techniques (in the form of mechanically repetitive exercises or kinestethic activities) that would help novices in being proficient in this ask. We highlight specific augmented piano prototypes that may have attempted to address this problem. Some prototypes introduced an interactive or an augmented reality (AR) keyboard that lets users press piano keys with either one or both hands. Some prototypes introduced the use of a virtual tutor or agents that shows the user how to properly and timely press keys in the piano/keyboard. 
...
The Augmented Design to Embody a Piano Teacher (ADEPT) \cite{gerry2019adept} provides a first-person audiovisual perspective of the teacher to the tutor. In other works, learners usually follow the guide or walk-through by a tutor but in this study, they are guided by a tutor that is embodied in their Point Of View (POV). Learners wear a virtual glove that they see through a head-mounted display and follow the lead of a virtual tutor. This virtual tutor shows an actual person in a separate room, where their hand movements are recorded and projected as an embodiment to the vision of the learner. The work of \cite{goodwin2013key} took entertaining to the next level by using \textit{anime}\textendash inspired agents to teach piano. The agent used marker-technology for tracking and had piano roll visualization as well to guide the user. 

These studies have been designed with the intent of helping learners be able to position their fingers properly on a piano and be able to timely press the right keys at the right time. The papers describing these prototypes may have done limited user studies to validate their claims, however, there is enough contribution to help novices in this learning theme. While ensuring proper finger positioning sets to describe a rather mechanical process, being able to properly play the piano requires a cognitive task that plays well along with this theme. 
\todok{Please give feedback. Thank you!}
...
Augmenting pianos with the intent of encouraging novices to continuously practice may be a recent trend among these prototypes. In the traditional setup of learning the piano, a novice learner attends classes and is tutored by an experienced teacher. In these sessions, the learner gains fundamental theories and lessons that improve their skill. Outside of these sessions, the teacher provides the learner with \textit{take home activities} where the learner is expected to practice and master the content in preparation for the next session. On most cases, beyond the structured curriculum of a teacher especially when a learner has acquired the necessary skills to be able to practice on their own, their skills can only be improved by further practicing and learning more advanced pieces of music. In such scenarios, monitoring their progress, tracking their performance and accuracy, and encouraging a regular lifelong learning process becomes out of scope of the teacher. Thus, these activities are within the personal bounds of the learner given their motivation and available resources. 
...
\todoj{recalibrate this discussion to fit the latest user study table. the table has been updated just need to fix this discussion to match that table }
Of the 61 papers included in this review, 30 (49\% as seen in Table \ref{tab:overview}) have performed user studies on the system  as part of their contribution. These user studies evaluated certain factors such as ease of use, satisfaction, immersion, motivation and performance among many others. We believe that through the development of augmented piano prototypes, techniques and methods on evaluating these factors have changed and improved as well. Following the data gathering techniques described above, we reviewed these papers and analyzed how they did their user studies. We looked at their sample size, the type of their participants, the type of treatment, the metrics or constructs they measured and the tools/instruments they used to measure these metrics. In our review, we observed a shift of focus in treatment and metrics in the same way the shift in focus for visualizations and agents/tutors have taken place. 
